...
class TitleState extends MusicBeatState
{
	...
	function addIntroElements() {
			if (FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('freakyMenu'), 0);
			}
			logoBl = new FlxSprite(logoPosition.x, logoPosition.y);
			trace('Loading atlas: logoBumpin');
			...
			trace('Loading atlas: ' + characterImage);
			...
			trace('Loading atlas: titleEnter');
	...
	public function startVideo(name:String)
    {
		...
		if (video.load(filepath)) {
            video.onEndReached.add(function() {
                ...
                else if (FlxG.stage.contains(video)) FlxG.stage.removeChild(video);
                video.dispose();
                #if VIDEOS_ALLOWED
                introVideo = null;
                #end
                // Start menu music before switching state
...
return;
	...
	{
		persistentUpdate = true;
		// Removed music play from here so it only plays after the video
		loadJsonData();
	...
	function getIntroTextShit():Array<Array<String>>
	{
		...
		// Add your custom message as the first intro text
		swagGoodArray.push(["Charzitian is a femboy"]);
	...
	{
		// Only update Conductor if music is playing
		if (FlxG.sound.music != null)
...
#if VIDEOS_ALLOWED
		// Center and scale the intro video if it's playing
		if (introVideo != null && introVideo.parent != null) {
			var stageW = FlxG.stage.stageWidth;
			var stageH = FlxG.stage.stageHeight;
			var vidW = 1920;
			var vidH = 1080;
			if (vidW > 0 && vidH > 0) {
				var scale = Math.min(stageW / vidW, stageH / vidH);
				introVideo.width = vidW * scale;
				introVideo.height = vidH * scale;
				introVideo.x = (stageW - introVideo.width) / 2;
				introVideo.y = (stageH - introVideo.height) / 2;
			}

			// Allow skipping the intro video by pressing Enter
			if (FlxG.keys.justPressed.ENTER || controls.ACCEPT) {
				introVideo.stop();
				if (introVideo.parent != null) introVideo.parent.removeChild(introVideo);
				else if (FlxG.stage.contains(introVideo)) FlxG.stage.removeChild(introVideo);
				introVideo.dispose();
				introVideo = null;
				FlxG.sound.playMusic(Paths.music('freakyMenu'), 0.7);
				persistentUpdate = true;
	...
	override function beatHit()
	{
		...
		if(!closedState)
		{
			...
			switch (sickBeats)
			{
				...
				case 2:
					createCoolText(['Charzitian is a'], 40);
				case 4:
					addMoreText('Femboy', 40);
					addMoreText('Guess we knew it', 40);
				case 5:
					deleteCoolText();
				case 6:
